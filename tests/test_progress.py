...
from mixture2d.simulator import _TerminalProgress, _format_duration
...
class ProgressTests(unittest.TestCase):
    ...
    def test_progress_reaches_completion(self):
        stream = io.StringIO()
        with mock.patch("mixture2d.simulator.time.monotonic", side_effect=(100.0, 102.0, 105.0)):
            with _TerminalProgress(2, label="fluid", enabled=True, stream=stream, width=4) as progress:
                progress.advance()
                progress.advance()
    ...
    def test_disabled_and_zero_progress_are_silent(self):
        for total, enabled in ((2, False), (0, True)):
            with self.subTest(total=total, enabled=enabled):
                stream = io.StringIO()
                with _TerminalProgress(total, label="fluid", enabled=enabled, stream=stream) as progress:
                    progress.advance()
    ...
    def test_exception_adds_newline_without_suppressing_error(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "stopped"):
            with _TerminalProgress(2, label="fluid", enabled=True, stream=stream) as progress:
                progress.advance()
