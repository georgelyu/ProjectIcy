"""Configuration for the non-thermal 2D ice--two-phase-flow example."""
...
Mode = Literal["coupled"]
Material = Literal["ice"]
...
class IceFlowConfig:
    ...
    Defaults deliberately reproduce the geometry and physical parameters of
    ``mixture2d/examples/dam_break_2d.py --mode coupled --material ice``.  There are no
    MPM, porosity/epsilon, temperature, or phase-change parameters here.
    """
    ...
    # The same physical two-phase parameters as mixture2d.DamBreakConfig.
    ...
    # One freely translating and rotating rigid ice rectangle.
    ...
    # Coupling filters and rigid-wall collision controls.  Speeds are lattice
    # speeds per LBM step; the caps preserve the low-Mach operating range.
    hydrodynamic_force_scale: float = 1.0
    hydrostatic_buoyancy_scale: float = 1.0
    force_relaxation: float = 0.20
    ...
    def __post_init__(self) -> None:
        if self.mode != "coupled":
            raise ValueError("mode must be 'coupled'")
        if self.material != "ice":
            raise ValueError("material must be 'ice'")
        ...
        if isinstance(nx, bool) or isinstance(ny, bool) or int(nx) != nx or int(ny) != ny:
        ...
        if isinstance(self.reference_length_cells, bool) or int(self.reference_length_cells) != self.reference_length_cells:
        ...
        if isinstance(self.boundary_cells, bool) or int(self.boundary_cells) != self.boundary_cells:
        ...
        if isinstance(self.phase_warmup_steps, bool) or int(self.phase_warmup_steps) != self.phase_warmup_steps:
        ...
        initial_omega = _finite("ice_initial_angular_velocity", self.ice_initial_angular_velocity)

        for name in ("hydrodynamic_force_scale", "hydrostatic_buoyancy_scale"):
            if _finite(name, getattr(self, name)) < 0.0:
                raise ValueError(f"{name} must be non-negative")
        relaxation = _finite("force_relaxation", self.force_relaxation)
        ...
        if abs(initial_omega) > max_omega:
            raise ValueError("ice_initial_angular_velocity exceeds max_ice_angular_speed")
        ...
        interface_cutoff = _finite(
            "volume_projection_interface_cutoff", self.volume_projection_interface_cutoff
        )
        ...
        if (
            ...
            or int(self.volume_projection_max_iterations) != self.volume_projection_max_iterations
        ):
            raise ValueError("volume_projection_max_iterations must be an integer")
        self.volume_projection_max_iterations = int(self.volume_projection_max_iterations)
        if self.volume_projection_max_iterations < 1:
            raise ValueError("volume_projection_max_iterations must be positive")

        if self.water_width <= self.boundary_cells or self.water_height <= self.boundary_cells:
            raise ValueError("water block must contain at least one interior cell")
        if self.water_width > nx - self.boundary_cells or self.water_height > ny - self.boundary_cells:
            raise ValueError("water block must fit inside the container walls")
        if self.ice_width <= 1 or self.ice_height <= 1:
            raise ValueError("ice rectangle must be at least two cells in each direction")

        # Check the initially rotated rectangle against the container.  The
        # default bottom is exactly y=3, matching the legacy ice example.
        ...
        if cx - extent_x < self.boundary_cells or cx + extent_x > nx - self.boundary_cells:
            raise ValueError("initial ice rectangle overlaps a side wall")
        if cy - extent_y < self.boundary_cells - 1.0e-9 or cy + extent_y > ny - self.boundary_cells:
        ...
        for name, viscosity in (
            ...
            if not math.isfinite(tau) or not 0.5 < tau <= 2.0:
                raise ValueError(f"{name} gives an unstable lattice relaxation time ({tau})")
        radius = math.hypot(self.ice_width * 0.5, self.ice_height * 0.5)
        if max_speed + max_omega * radius > 0.25:
            raise ValueError("rigid-body caps permit a boundary speed above 0.25 lattice cells/step")
    ...
    def ice_initial_center(self) -> tuple[float, float]:
        ...
        return (left_cell + 0.5 * self.ice_width, float(self.ice_base_y_cells) + 0.5 * self.ice_height)
...
# The more specific name is preferred.  Aliases make migration from the
# existing dam-break example straightforward without importing mixture2d.
DamBreakConfig = IceFlowConfig
...
create_dambreak_config = create_iceflow_config
