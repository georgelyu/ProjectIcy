...
\author{}
\date{对应当前 \code{iceflow2d/simulator.py} 与 \code{iceflow2d/config.py}}
...
\subsection{GPU 存储与并行归约}

物理状态与其廉价派生量分别处理：材料初始质量是 CPU 常数，固相率由当前质量求得，
冰水温度由质量、液水面积和显热求得，材料插值索引由位姿求得。墙掩膜由坐标边界比较得到，
SDF 由唯一覆盖率光栅与位姿求得。世界温度、液相率和体积焓只在输出采样时合成。
\code{body\_center} 也是参考原点与材料质心旋转的只读视图，不再有独立 GPU 存储。

两组 population 各保留当前状态和流送缓冲；它们具有不同时间层，不能原地混用。
宏观相场、动量速度、压力与启闭掩膜保留碰撞、流送和 refill 所需的冻结快照。
世界水的面能量与体积通量必须保留，避免更新水状态时读取其他线程已覆盖的迎风源。
冰内导热在质量与显热尚未改变时直接求邻面通量，因此不需要冰面通量数组。

高开销的标量归约使用 CUDA warp 内 f64 树形求和；f64 shuffle 按位搬运两个 32 位字，
不会把归约值降到 f32。水开口测量先输出独立 warp 的六项部分结果，再归约到全域统计量，
以减少对单个全局地址的原子竞争。遍历范围补齐到 128 线程块，补齐单元贡献为相应恒等元。
完整的字段清单、保留理由、验证方式与性能结果见 \path{iceflow2d/docs/gpu_state_review.md}。
...
$\phi$ & \code{water\_phase} & 空气--水相场；水约为 1，空气约为 0 \\
$f_q,h_q$ & \code{momentum\_populations}, \code{phase\_populations} & 压力--动量分布与相场分布 \\
$\vect u,\vect u^\star$ & \code{momentum\_velocity\_lattice}, \code{u + force/2} & 存储速度与半力物理速度 \\
$p^H,p^d,p$ & hydrostatic/reference, \code{pressure\_lattice} & 静水参考、动态压力及总压力，$p=p^H+p^d$ \\
$C_P$ & \code{world\_body\_solid\_fraction} & 当前位姿下连续冰覆盖率 \\
$\chi_P^s$ & \code{solid\_mask} & 由 $C_P$ 阈值化得到的锐冰掩膜 \\
$d_P$ & \code{body\_signed\_distance\_m} & 冰界有符号距离；共享世界 SDF 的物理单位为米 \\
...
\State \code{\_collide\_momentum()}：计算力、黏度和 $f_q^\star$
...
\code{\_collide\_momentum()} 中进入热浮力，因此没有同一步内的隐式反馈。}
\label{fig:step-flow}
\end{figure}
...
\end{equation}
式中所有分布都来自 $P$，不读取后退点 $P-\vect c_q$；代码在碰撞末尾、原分布仍可读时计算反射值，
把它写入该 cut-link 不再用于直通流送的方向槽。无需缓存整张非平衡分布，且避免了流送读写竞争。壁速已完整包含在
...
\code{body\_signed\_distance\_m} 场；cut-link 只使用距离比，因此单位相消。
...
$|u_x^\star|+|u_y^\star|\le1$；水侧 CFL 在 $\phi>\varepsilon_\phi$ 的合法格统计各有效面的总流出速率。
该 LBM 状态扫描默认关闭；半力速度按需计算，流出速率归约每步执行。
物理对流子步上限与子步数为
\begin{equation}
  \Delta t_{\rm adv,max}
  =Co_{\max}\frac{\Delta t_{\rm LBM}}
  {\max_P\sum_{f\in\partial P}\max(0,\vect u_f^\star\cdot\vect n_{P,f})},
  \qquad
...
\end{equation}
扩散核按需由式~\eqref{eq:body-thermal-state} 和~\eqref{eq:water-extensive-state} 求温度，不保存重复温度数组。
...
\end{equation}
该式在设备读取温度时直接求值，不需要额外的恢复 kernel 或持久温度 field，也未再次增加、移除或输运热量。
对于不超过面积容差的同步后液水面积 $V_P^{w,+}\le V_{\rm tol}$，温度读取回退为初始
空气温度；在采样时，CPU 合成世界显示温度、液相率和诊断体积焓 $H_P$，材料冰与墙按显示优先级覆盖。
下一 LBM 步的浮力核直接从水显热和液水面积计算水温。
