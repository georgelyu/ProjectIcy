...
\begin{description}
  ...
  是否参加 LBM。连续覆盖率仍用于热接触和 ALE，不能与锐掩膜混为一谈。
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}lY@{}}
    ...
    热边界 & 左、右、底壁恒温 $90\,^{\circ}\mathrm C$；顶壁绝热；水--气与冰--气绝热 \\
    输出 & 总时长 $2\,\mathrm s$，每 $0.01\,\mathrm s$ 采样 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\end{equation}
每个输出间隔含 1600 个 LBM 步，终点为 320000 步；若计入 $t=0$，共 201 个采样。
...
\begin{tikzpicture}[node distance=3.3mm]
  ...
  \node[flow,below=of g] (f) {FAST：覆盖率 ALE $\rightarrow$ 热水 aperture $\rightarrow$ 成对 $V^w/S^w$ 迎风对流};
...
新值保存为 $C_P^{\rm new}$，供 FAST 的扫掠覆盖率 ALE 使用。材料矩形内部的 SDF 为
\begin{equation}
...
\subsection{覆盖率 ALE 守恒重映射}

令旧/新连续覆盖为 $C_P^o,C_P^n$。当冰扫入某格时，移走比例
\begin{equation}
  r_P=\min\left[1,
  \frac{(C_P^n-C_P^o)_+}{\max(1-C_P^o,\varepsilon)}\right].
\end{equation}
水面积和显热使用同一比例
\begin{equation}
  \Delta V_P=r_PV_P^w,
  \qquad \Delta S_P=r_PS_P^w.
  \label{eq:ale-remove}
\end{equation}
汇总 $V_{\rm rem}=\sum_P\Delta V_P$、$S_{\rm rem}=\sum_P\Delta S_P$。首选释放区域权重
\begin{equation}
  a_P=\cellarea(C_P^o-C_P^n)_+,
\end{equation}
且只保留合法水侧；若没有释放区域，退化为开放水相权重
$a_P=\cellarea\clip(\phi_P,0,1)$。重分配为
\begin{equation}
  V_P^\star=V_P^w-\Delta V_P+\frac{a_P}{\sum_Ra_R}V_{\rm rem},
  \qquad
  S_P^\star=S_P^w-\Delta S_P+\frac{a_P}{\sum_Ra_R}S_{\rm rem}.
  \label{eq:ale-redistribute}
\end{equation}
于是纯位姿变化在归约误差内保持 $\sum V^w$ 与 $\sum S^w$，并因共用权重而保持均匀
$S^w/V^w$。这是全局守恒的一阶覆盖率重映射，不等同于逐面高阶几何守恒律格式。

\subsection{成对液水面积--显热迎风对流}

ALE 后先执行一次第~\ref{sec:aperture} 节的 aperture 同步，保证上游状态合法。对相邻合法水格
...
\end{equation}
绝热壁功率为零。当前默认左右和底壁是 $90\,^{\circ}\mathrm C$ 恒温壁，顶壁绝热；空气没有
...
对当前液水面积 $V_P^w$ 大于目标面积 $D_P$ 的供体单元，按目标面积与当前面积之比
$D_P/V_P^w$ 同比例移除水和显热，将移出的总水面积与总显热汇入公共池；随后按照目标面积
$D_P$ 与当前液水面积 $V_P^w$ 之间的接收缺口比例，把池中两种广延量同时分配给接收单元。
...
\end{equation}
前两条等式来自“供体移出的量全部进入接收单元”的封闭池转移，最后一条界限则来自有界目标及
不越过目标的比例分配；这些关系在精确算术下成立，实际浮点归约误差另存为守恒残量诊断。
水面积与显热共用移除和接收权重，因此还会保持原本空间均匀的水显热--面积比
$S_P^w/V_P^w$，也就是保持均匀水温不变。
...
刚体每一步都会移动，所以覆盖率 ALE 不能把八步位姿变化压成一次首尾差；否则中间扫掠过的水量
和显热会丢失。导热和相变变化较慢，可累计物理时间后按 Fourier 条件再分子步。水显热对流仍必须
每个 LBM 步跟随瞬时速度执行。
...
\begin{align}
  ...
  R_V^{\rm ALE}&=V_{\rm assigned}-V_{\rm removed},
  &R_S^{\rm ALE}&=S_{\rm assigned}-S_{\rm removed},\\
...
\begin{enumerate}
  ...
  \item 查 $R_V^{\rm ALE},R_S^{\rm ALE}$，判断位姿扫掠重映射；
...
材料网格负责固冰质量和显热；世界热网格负责液水面积和显热。四类守恒修正---覆盖率 ALE、相场
水量投影、融水三矩 lift 和 aperture 同步---分别作用于不同未知量，不能相互替代。严格保持
